...
_COVERS_SCHEMA = vol.All(cv.ensure_list, [vol.Schema({
    CONF_NAME: cv.string,
...
PLATFORM_SCHEMA = \
    PLATFORM_SCHEMA.extend({vol.Required(CONF_COVERS): _COVERS_SCHEMA,
                           vol.Optional(CONF_STATE_PULL_MODE,
                           default=DEFAULT_STATE_PULL_MODE): cv.string,
                           vol.Optional(CONF_RELAY_TIME,
                           default=DEFAULT_RELAY_TIME): cv.positive_int})
...
def setup_platform(
    ...
    for cover in covers_conf:
        covers.append(RPiGPIOCover(
            cover[CONF_NAME],
            cover[CONF_RELAY_PIN_UP],
            cover[CONF_RELAY_PIN_DOWN],
